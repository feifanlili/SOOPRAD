...
class GA_Optimizer:       
    ...
    def optimize(self):
        ...
        for g in range(self.params['maxiter']):
            ...
            for child1, child2 in zip(offspring[::2], offspring[1::2]):
                if random.random() < self.params['mutationPB']:
            ...
            for mutant in offspring:
                if random.random() < self.params['crossoverPB']:
            ...
            if stagnation_count >= self.params['stagnation_limit'] or \
               abs(current_best - min(fitness_values)) < self.params['epsilon']:
